    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        html! {
            <div class="flex w-screen">
                <div class="flex-none w-56 h-screen bg-emerald-300 border-b-2 border-gray-300">
                    <div class="text-xl p-3 font-bold">{"Users"}</div>
                    {
                        self.users.clone().iter().map(|u| {
                            html!{
                                <div class="flex m-3 bg-slate-900 rounded-lg p-2">
                                    <div>
                ...
                <div class="grow h-screen flex flex-col">
                    <div class="w-full h-14 border-b-2 border-gray-300 bg-slate-900"><div class="text-xl p-3 text-white font-medium">{"💬 Connect with YewChat!"}</div></div>
                    <div class="w-full grow overflow-auto border-b-2 border-gray-300 bg-cyan-100">
                        {
                            self.messages.iter().map(|m| {
                                let user = self.users.iter().find(|u| u.name == m.from).unwrap();
                                html!{
                                    <div class="flex items-end w-3/6 bg-gray-100 m-8 rounded-tl-lg rounded-tr-lg rounded-br-lg bg-slate-900">
